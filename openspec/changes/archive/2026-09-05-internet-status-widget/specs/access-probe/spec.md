...
Starting a check MUST go through one pipeline that accepts a refresh trigger and returns group verdicts or no-network. This change MUST provide a tap trigger. Network-change and periodic triggers are out of scope for this change. The pipeline MUST accept additional triggers later without changing verdict rules.
